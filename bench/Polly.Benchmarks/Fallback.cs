...
public class Fallback
{
    ...
    [Benchmark]
    public static int Fallback_Synchronous_Succeeds() =>
        SyncPolicy.Execute(() => Workloads.Func<int>());

    [Benchmark]
    public static Task<int> Fallback_Asynchronous_Succeeds() =>
        AsyncPolicy.ExecuteAsync(() => Workloads.FuncAsync<int>());

    [Benchmark]
    public static int Fallback_Synchronous_Throws() =>
        SyncPolicy.Execute(() => Workloads.FuncThrows<int, InvalidOperationException>());

    [Benchmark]
    public static Task<int> Fallback_Asynchronous_Throws() =>
