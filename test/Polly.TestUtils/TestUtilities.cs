...
public static class TestUtilities
{
    ...
    public static ResilienceStrategyTelemetry CreateResilienceTelemetry(DiagnosticSource source)
        => new(new ResilienceTelemetrySource("dummy-builder", "dummy-instance", "strategy-name"), source);

    public static ResilienceStrategyTelemetry CreateResilienceTelemetry(Action<TelemetryEventArguments> callback)
        => new(new ResilienceTelemetrySource("dummy-builder", "dummy-instance", "strategy-name"), new CallbackDiagnosticSource(callback));
...
#pragma warning restore S107 // Methods should not have too many parameters
    {
        source.Write(resilienceEvent.EventName, TelemetryEventArguments.Get(
            new ResilienceTelemetrySource(builderName, instanceName, strategyName),
            resilienceEvent,
            context,
            outcome,
            arguments));
