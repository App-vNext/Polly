...
namespace Polly
{
    public partial class Policy
    {
        ...
        public static AsyncCachePolicy<TResult> CacheAsync<TResult>(IAsyncCacheProvider cacheProvider, TimeSpan ttl)
        {
            if (cacheProvider == null) throw new ArgumentNullException(nameof(cacheProvider));

            return CacheAsync(cacheProvider.AsyncFor<TResult>(), new RelativeTtl(ttl).For<TResult>(), _ => { });
        }
        ...
        public static AsyncCachePolicy<TResult> CacheAsync<TResult>(IAsyncCacheProvider cacheProvider, ITtlStrategy ttlStrategy)
        {
            if (cacheProvider == null) throw new ArgumentNullException(nameof(cacheProvider));

            return CacheAsync(cacheProvider.AsyncFor<TResult>(), ttlStrategy.For<TResult>(), _ => { });
        }
