...
namespace Polly
{
    public partial class Policy
    {
        ...
        {
            Func<Context, Task> doNothingAsync = _ => CompletedTask;
        ...
        /// <returns>The policy instance.</returns>
        public static BulkheadPolicy<TResult> BulkheadAsync<TResult>(int maxParallelization, Func<Context, Task> onBulkheadRejectedAsync) => BulkheadAsync<TResult>(maxParallelization, 0, onBulkheadRejectedAsync);
        ...
        public static BulkheadPolicy<TResult> BulkheadAsync<TResult>(int maxParallelization, int maxQueuingActions, Func<Context, Task> onBulkheadRejectedAsync)
        {
            ...
            var maxParallelizationSemaphore = SemaphoreSlimFactory.CreateSemaphoreSlim(maxParallelization);

            var maxQueuingCompounded = maxQueuingActions <= int.MaxValue - maxParallelization
                ? maxQueuingActions + maxParallelization
                : int.MaxValue;
            var maxQueuedActionsSemaphore = SemaphoreSlimFactory.CreateSemaphoreSlim(maxQueuingCompounded);
