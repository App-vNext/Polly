...
{
    public class BulkheadAsyncSpecs : BulkheadSpecsHelper
    {
        ...
        public void Should_call_onBulkheadRejected_with_passed_context()
        {
            ...
            Func<Context, Task> onRejectedAsync = async ctx => { contextPassedToOnRejected = ctx; await Task.CompletedTask.ConfigureAwait(false); };
            ...
            using (var cancellationSource = new CancellationTokenSource())
            {
                ...
                bulkhead.Awaiting(async b => await b.ExecuteAsync(ctx => Task.CompletedTask, contextPassedToExecute)).ShouldThrow<BulkheadRejectedException>();
            ...
            bool cancelExecuting, string scenario)
        {
            ...
            for (var i = 0; i < totalActions; i++) { actions[i] = new TraceableAction(i, statusChanged, testOutputHelper); }

            // Throw all the delegates at the bulkhead simultaneously.
            var tasks = new Task[totalActions];
            for (var i = 0; i < totalActions; i++) { tasks[i] = actions[i].ExecuteOnBulkheadAsync(bulkhead); }
            ...
            var expectedExecuting = Math.Min(totalActions, maxParallelization);
            var expectedRejects = Math.Max(0, totalActions - maxParallelization - maxQueuingActions);
            var expectedQueuing = Math.Min(maxQueuingActions, Math.Max(0, totalActions - maxParallelization));
            var expectedBulkheadFree = maxParallelization - expectedExecuting;
            var expectedQueueFree = maxQueuingActions - expectedQueuing;
