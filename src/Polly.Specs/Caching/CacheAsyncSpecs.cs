...
namespace Polly.Specs.Caching
{
    ...
    public class CacheAsyncSpecs : IDisposable
    {
        ...
        public async Task Should_execute_delegate_and_put_value_in_cache_if_cache_does_not_hold_value()
        {
            ...
            (await cache.ExecuteAsync(async ctx => { await Task.CompletedTask.ConfigureAwait(false); return valueToReturn; }, new Context(operationKey)).ConfigureAwait(false)).Should().Be(valueToReturn);
        ...
        public async Task Should_allow_custom_FuncCacheKeyStrategy()
        {
            ...
            Func<Context, Task<object>> func = async ctx => { funcExecuted = true; await Task.CompletedTask.ConfigureAwait(false); return new object(); };
        ...
        public void Should_always_execute_delegate_if_execution_is_void_returning()
        {
            ...
            var delegateInvocations = 0;
            Func<Context, Task> action = async ctx => { delegateInvocations++; await Task.CompletedTask.ConfigureAwait(false); };
        ...
        public async Task Should_execute_oncachemiss_and_oncacheput_if_cache_does_not_hold_value_and_put()
        {
            ...
            ((string)await stubCacheProvider.GetAsync(operationKey, CancellationToken.None, false).ConfigureAwait(false)).Should().BeNull();
            (await cache.ExecuteAsync(async ctx => { await Task.CompletedTask.ConfigureAwait(false); return valueToReturn; }, contextToExecute).ConfigureAwait(false)).Should().Be(valueToReturn);
