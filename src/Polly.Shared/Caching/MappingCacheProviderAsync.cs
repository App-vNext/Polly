...
namespace Polly.Caching
{
    ...
    /// </summary>
    public abstract class MappingCacheProviderAsync<TNative, TMapped> : ICacheProviderAsync<TNative>
    {
        private readonly ICacheProviderAsync<TMapped> _wrappedCacheProvider;

        /// <summary>
        /// Initializes a new instance of the <see cref="MappingCacheProviderAsync{TNative, TMapped}"/> class.
        /// </summary>
        /// <param name="wrappedCacheProvider">The wrapped cache provider.</param>
        protected MappingCacheProviderAsync(ICacheProviderAsync<TMapped> wrappedCacheProvider)
        {
            if (wrappedCacheProvider == null) throw new ArgumentNullException(nameof(wrappedCacheProvider));
        ...
        /// <param name="continueOnCapturedContext">if set to <c>true</c>, the implementation should continue on captured context whenever it awaits.</param>
        /// <returns>The object mapped back to native format.</returns>
        public abstract Task<TNative> GetMapper(TMapped rawObjectFromCache, bool continueOnCapturedContext);
        ...
        /// <param name="continueOnCapturedContext">if set to <c>true</c>, the implementation should continue on captured context whenever it awaits.</param>
        /// <returns>The object mapped to caching format.</returns>
        public abstract Task<TMapped> PutMapper(TNative nativeObject, bool continueOnCapturedContext);
        ...
        /// <param name="continueOnCapturedContext">Whether async calls should continue on a captured synchronization context.</param>
        /// <returns>A <see cref="Task{TResult}" /> promising as Result the value from cache; or null, if none was found.</returns>
        public async Task<TNative> GetAsync(string key, CancellationToken cancellationToken, bool continueOnCapturedContext)
        {
            return await GetMapper(
                await _wrappedCacheProvider.GetAsync(key, cancellationToken, continueOnCapturedContext).ConfigureAwait(continueOnCapturedContext),
                continueOnCapturedContext
