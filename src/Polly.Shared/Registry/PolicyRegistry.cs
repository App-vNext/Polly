...
namespace Polly.Registry
{
    ...
    public class PolicyRegistry : IPolicyRegistry<string>, IReadOnlyPolicyRegistry<string>
    {
        ...
        #region IReadOnlyPolicyRegistry members
        /// <summary>
        /// Total number of policies in the registry.
        /// </summary>
        public int IReadOnlyPolicyRegistry<TKey>.Count => _registry.Count;
        ...
        /// <exception cref="KeyNotFoundException">The given key was not present in the dictionary.</exception>
        /// <returns>The value associated with the specified key.</returns>
        public IsPolicy IReadOnlyPolicyRegistry<TKey>.this[string key]
        {
            get
            {
                return _registry[key];
            ...
            /// <exception cref="ArgumentNullException"><paramref name="key"/> is null.</exception>
            /// <exception cref="KeyNotFoundException">The given key was not present in the dictionary.</exception>
            public TPolicy IReadOnlyPolicyRegistry<TKey>.Get<TPolicy>(string key) where TPolicy : IsPolicy
        {
            return (TPolicy)_registry[key];
        }
        ...
        /// <typeparam name="TPolicy">The type of Policy.</typeparam>
        /// <returns>True if Policy exists for the provided Key. False otherwise.</returns>
        public bool IReadOnlyPolicyRegistry<TKey>.TryGet<TPolicy>(string key, out TPolicy policy) where TPolicy : IsPolicy
        {
            IsPolicy value;
            bool got = _registry.TryGetValue(key, out value);
            policy = got ? (TPolicy)value : default(TPolicy);
        ...
        /// <returns>True if <paramref name="key"/> exists otherwise false.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="key"/> is null.</exception>
        public bool IReadOnlyPolicyRegistry<TKey>.ContainsKey(string key) =>
            _registry.ContainsKey(key);
        #endregion
    }
}
