...
namespace Polly.Specs
{
    public class CircuitBreakerTResultAsyncSpecs : IDisposable
    {
        ...
        public void Should_close_circuit_again_on_reset_after_manual_override()
        {
            ...
            breaker.Reset();
            breaker.CircuitState.Should().Be(CircuitState.Closed);
            breaker.Awaiting(x => x.ExecuteAsync(() => Task.FromResult(ResultPrimitive.Good))).ShouldNotThrow();
        }
        ...
        [Fact]
        public async void Should_call_onbreak_when_breaking_circuit_first_time_but_not_for_subsequent_calls_through_open_circuit()
        {
            int onBreakCalled = 0;
