...
When designing resilient systems, a common pattern is to use a fallback after multiple retry attempts. This approach is especially relevant when a fallback strategy can provide a sensible default value.
...
// Execution for demonstration purposes that always returns an invalid result
pipeline.Execute(() => new HttpResponseMessage(HttpStatusCode.InternalServerError));
```
<!-- endSnippet -->

Here's a breakdown of the behavior when the callback produces either an `HttpStatusCode.InternalServerError` or a `HttpRequestException`:
