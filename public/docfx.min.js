...
`)})}function Zo(c){localStorage.setItem("theme",c),c==="auto"?document.documentElement.setAttribute("data-bs-theme",window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):document.documentElement.setAttribute("data-bs-theme",c)}async function Xo(){return localStorage.getItem("theme")||(await D()).defaultTheme||"auto"}async function Co(){Zo(await Xo())}function Go(c){return new MutationObserver(()=>c(Re())).observe(document.documentElement,{attributes:!0,attributeFilter:["data-bs-theme"]})}function Re(){return document.documentElement.getAttribute("data-bs-theme")}async function Bo(c){let t=await Xo(),i=t==="light"?"sun":t==="dark"?"moon":"circle-half";return x`
    <div class='dropdown'>
      ...
      <ul class='dropdown-menu dropdown-menu-end'>
        ...
        </div>`;return x`${Z} ${d()}`}function d(){let s=Math.max(0,Math.min(r-2,g-5)),X=Math.min(g,s+5),Z=Array.from(new Array(X-s).keys()).map(G=>G+s);if(Z.length<=1)return null;return x`
        <nav>
          <ul class="pagination">
            <li class="page-item">
      ...
      </form>`;function Z(B){a=B.target.value.trim(),localStorage?.setItem("tocFilter",a),C()}}function s(){return a===""?null:x`<div class='no-result'>${L("searchNoResults",{query:a})}</div>`}function X(){return b?x`<div class="py-2 mb-md-4"><a class="pdf-link" href="${new URL(o||"toc.pdf",i)}">${L("downloadPdf")}</a></div>`:null}}function Jr(c,t){let i=document.getElementById("nextArticle");if(!i)return;let e=r(c),b=e.findIndex(I=>I===t),o=e[b-1],l=e[b+1];if(!o&&!l)return;let a=o?x`<div class="prev"><span><i class='bi bi-chevron-left'></i> ${L("prevArticle")}</span> <a href="${o.href}" rel="prev">${ct(o.name)}</a></div>`:null,n=l?x`<div class="next"><span>${L("nextArticle")} <i class='bi bi-chevron-right'></i></span> <a href="${l.href}" rel="next">${ct(l.name)}</a></div>`:null;lt(x`${a} ${n}`,i);function r(I){let g=[];for(let C of I)C.href&&g.push(C),C.items&&g.push(...r(C.items));return g}}var Qo=No(Ro());async function ho(){let c=document.getElementById("navbar");if(!c)return[];let{iconLinks:t}=await D(),i=await a(),e=wr(i),b=n=>x`<li class='nav-item'><a class='nav-link ${n===e?"active":null}' aria-current=${n===e?"page":!1} href=${n.href}>${ct(n.name)}</a></li>`,o=x`
