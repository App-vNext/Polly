...
`)})}function Zo(i){localStorage.setItem("theme",i),i==="auto"?document.documentElement.setAttribute("data-bs-theme",window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):document.documentElement.setAttribute("data-bs-theme",i)}async function Xo(){return localStorage.getItem("theme")||(await D()).defaultTheme||"auto"}async function Co(){Zo(await Xo())}function Go(i){return new MutationObserver(()=>i(Re())).observe(document.documentElement,{attributes:!0,attributeFilter:["data-bs-theme"]})}function Re(){return document.documentElement.getAttribute("data-bs-theme")}async function Bo(i){let t=await Xo(),c=t==="light"?"sun":t==="dark"?"moon":"circle-half";return x`
    <div class='dropdown'>
      ...
      ${X()}`,n)}function g(Z){let B=Z.map(G=>{let{href:u,name:R,items:Q,expanded:p}=G,h=!Q||Q.length<=0,U=h?null:g(Q);if(a!==""&&!U&&!R.toLowerCase().includes(a.toLowerCase()))return null;let y=u?x`<a class='${ht({"nav-link":!I.includes(G)})}' href=${u}>${it(R)}</a>`:h?x`<span class='text-body-tertiary name-only'>${it(R)}</a>`:x`<a class='${ht({"nav-link":!I.includes(G)})}' href='#' @click=${T}>${it(R)}</a>`,O=a!==""&&p!==!1&&U!=null||p===!0;return x`
        <li class=${ht({expander:!h,expanded:O,active:I.includes(G)})}>
          ${h?null:x`<span class='expand-stub' @click=${T}></span>`}
          ${y}
          ${U}
        </li>`;function T(z){z.preventDefault(),G.expanded=!O,C()}}).filter(G=>G);return B.length>0?x`<ul>${B}</ul>`:null}function F(){return t?null:x`
      <form class='filter'>
    ...
    <ul class='navbar-nav'>${c.map(I=>{if("items"in I){let d=I.items.some(n=>n===e)?"active":null;return x`
      ...
      <form class="icons">
        ...
        ${await Bo(l)}
      </form>`;lt(x`${o} ${I}`,i)}return await l(),e?[e]:[];async function a(){let I=at("docfx:navrel");if(!I)return[];let d=new URL(I.replace(/.html$/gi,".json"),window.location.href),{items:n}=await fetch(d).then(s=>s.json());return n.map(s=>"items"in s?{name:s.name,items:s.items.map(C=>({name:C.name,href:new URL(C.href,d)}))}:{name:s.name,href:new URL(s.href,d)})}}function Vo(i){let t=document.getElementById("breadcrumb");t&&lt(x`
        <ol class="breadcrumb">
